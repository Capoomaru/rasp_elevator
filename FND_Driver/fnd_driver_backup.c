...
volatile unsigned char fnd_data[3][8] = {
	{0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00},	// 1층
	{0x01, 0x01, 0x00, 0x01, 0x01, 0x00, 0x01, 0x00},	// 2층
	{0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x01, 0x00}	// 3층
};
...
static int fnd_write(struct file *mfile, const char *gdata, size_t length, loff_t *off_what){
	...
	if(tmp_buf == 0x60) tmp_value = 1;
        else if(tmp_buf == 0xDA) tmp_value = 2;
	else if(tmp_buf == 0xF2) tmp_value = 3;	
	for(i = 0; i < 7; ++i){
		*(fnd + 10) = (0x01 << pin_num[i]);	
		*(fnd + 7) = (fnd_data[(unsigned int)tmp_value][i] << pin_num[i]);
	}
	return length;
}
