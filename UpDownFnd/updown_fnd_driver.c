...
#include <linux/io.h>
#define FND_MAJOR 223
#define FND_NAME "UPDOWN_FND_DRIVER"
...
volatile int pin_num[] = {7, 8, 9, 10, 11, 25};
volatile unsigned char fnd_data[3][8] = {
	{0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01},	// down
	{0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01},	// off
	{0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x01, 0x01}	// up
};
...
static int fnd_write(struct file *mfile, const char *gdata, size_t length, loff_t
*off_what){
	int i;
	char tmp_buf;
	int result;
	...
	for(i = 0; i < 6; ++i){
		...
		*(fnd + 7) = (fnd_data[(unsigned int)tmp_buf][i] << pin_num[i]);
	}
	return length;
}
